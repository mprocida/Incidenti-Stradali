...
class IncidentiMetaBoxes {
    ...
    public function __construct() {
        ...
        // Aggiungi l'azione di gruppo per pubblicare
        add_filter('bulk_actions-edit-incidente_stradale', array($this, 'add_bulk_publish_action'));
        
        // Gestisci l'azione di gruppo per pubblicare
        add_filter('handle_bulk_actions-edit-incidente_stradale', array($this, 'handle_bulk_publish_action'), 10, 3);
        
        // Mostra messaggi di conferma
        add_action('admin_notices', array($this, 'show_bulk_publish_admin_notices'));
    }
    ...
    * Lista delle Polizie Municipali della provincia di Lecce
    ...
    public function save_meta_boxes($post_id) {
        ...
        $meta_fields = array(
            ...
            'circostanza_tipo', 'condizioni_aggiuntive', 'circostanza_veicolo_a', 'circostanza_veicolo_b', 'difetto_veicolo_a', 'difetto_veicolo_b',
            'stato_psicofisico_a', 'stato_psicofisico_b', 'cilindrata_veicolo_a', 'cilindrata_veicolo_b',
        ...
        $special_fields = ['circostanza_veicolo_a', 'circostanza_veicolo_b', 
                  'difetto_veicolo_a', 'difetto_veicolo_b', 
                  'stato_psicofisico_a', 'stato_psicofisico_b'];
    ...
    /**
     * Aggiunge l'azione "Pubblica" alle azioni di gruppo
     */
    public function add_bulk_publish_action($bulk_actions) {
        // Aggiungi solo se l'utente ha i permessi necessari
        if (current_user_can('publish_posts')) {
            $bulk_actions['bulk_publish'] = __('Pubblica', 'incidenti-stradali');
        }
     ...
     */
    public function handle_bulk_publish_action($redirect_to, $doaction, $post_ids) {
        // Verifica se questa è l'azione che ci interessa
        if ($doaction !== 'bulk_publish') {
            return $redirect_to;
        }
        
        // Verifica permessi
        if (!current_user_can('publish_posts')) {
            wp_die(__('Non hai i permessi per pubblicare incidenti.', 'incidenti-stradali'));
        }
        ...
        foreach ($post_ids as $post_id) {
            // Verifica che sia effettivamente un incidente stradale
            if (get_post_type($post_id) !== 'incidente_stradale') {
                continue;
            }
            ...
            $post = get_post($post_id);
            if (!$post) {
                $errors[] = sprintf(__('Incidente #%d non trovato', 'incidenti-stradali'), $post_id);
                continue;
            }
            ...
            if ($post->post_status !== 'draft') {
                $errors[] = sprintf(__('Incidente #%d non è in bozza (stato attuale: %s)', 'incidenti-stradali'), $post_id, $post->post_status);
                continue;
            }
            
            // Verifica permessi specifici per questo post
            ...
            if (!$this->validate_incident_for_publishing($post_id)) {
                $errors[] = sprintf(__('Incidente #%d: dati incompleti, impossibile pubblicare', 'incidenti-stradali'), $post_id);
                continue;
            }
            
            // Pubblica l'incidente
            ...
            if (!is_wp_error($result) && $result !== 0) {
                ...
                // Trigger hook per notifiche o azioni aggiuntive
                do_action('incidenti_after_bulk_publish', $post_id, $post);
            } else {
                $error_message = is_wp_error($result) ? $result->get_error_message() : 'Errore sconosciuto';
                $errors[] = sprintf(__('Errore pubblicando incidente #%d: %s', 'incidenti-stradali'), $post_id, $error_message);
            }
        }
        
        // Aggiungi parametri di risultato all'URL di redirect
        $redirect_to = add_query_arg(array(
            'bulk_published' => $published_count,
            'bulk_already_published' => $already_published,
            'bulk_publish_errors' => count($errors)
        ), $redirect_to);
        
        // Salva gli errori in un transient se presenti
        if (!empty($errors)) {
            set_transient('incidenti_bulk_publish_errors_' . get_current_user_id(), $errors, 60);
        }
    ...
    /**
     * Valida se un incidente può essere pubblicato
     */
    private function validate_incident_for_publishing($post_id) {
        ...
        // Verifica che abbia coordinate valide se richiesto
        $latitudine = get_post_meta($post_id, 'latitudine', true);
        $longitudine = get_post_meta($post_id, 'longitudine', true);
        
        // [Inferenza] Se il sistema richiede coordinate obbligatorie, verifica che siano presenti
        if (get_option('incidenti_require_coordinates', false)) {
            if (empty($latitudine) || empty($longitudine)) {
                return false;
            }
        }
    ...
    /**
     * Mostra i messaggi di risultato dell'operazione
     */
    public function show_bulk_publish_admin_notices() {
        if (!isset($_REQUEST['bulk_published'])) {
            return;
        }
        
        $published_count = intval($_REQUEST['bulk_published']);
        $already_published_count = isset($_REQUEST['bulk_already_published']) ? intval($_REQUEST['bulk_already_published']) : 0;
        $errors_count = isset($_REQUEST['bulk_publish_errors']) ? intval($_REQUEST['bulk_publish_errors']) : 0;
        ...
        // Messaggio per quelli già pubblicati
        if ($already_published_count > 0) {
            $message = sprintf(
                _n(
                    '%d incidente era già pubblicato.',
                    '%d incidenti erano già pubblicati.',
        ...
        // Messaggio di errore
        if ($errors_count > 0) {
            $errors = get_transient('incidenti_bulk_publish_errors_' . get_current_user_id());
            
            echo '<div class="notice notice-error is-dismissible">';
            echo '<p><strong>' . __('Errori durante la pubblicazione:', 'incidenti-stradali') . '</strong></p>';
            ...
            if ($errors && is_array($errors)) {
                ...
                // Rimuovi il transient dopo averlo mostrato
                delete_transient('incidenti_bulk_publish_errors_' . get_current_user_id());
            } else {
                echo '<p>' . sprintf(__('%d errori durante l\'operazione.', 'incidenti-stradali'), $errors_count) . '</p>';
            }
